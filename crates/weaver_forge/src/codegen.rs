...
//! Template engine and code generation functionality.
//!
//! This module contains the minijinja-based template engine, code generation
//! pipeline, and related utilities. It is gated behind the `codegen` feature
//! because it depends on heavy libraries (minijinja, rayon, walkdir, etc.)
//! that are not compatible with WASM targets.
...
use crate::{
    BLOCK_END, BLOCK_START, COMMEND_END, COMMENT_START, SEMCONV_JQ, VARIABLE_END, VARIABLE_START,
    WEAVER_YAML,
};
...
impl TemplateEngine {
    ...
    pub(crate) fn generate_snippet<T: Serialize>(
        ...
        let template = engine
            ...
            .map_err(crate::error::jinja_err_convert)?;
        let result = template
